...
namespace Chirp.Read.Domain.Specs.Chirping.when_message_chirped
{
    ...
    public class from_chirper : given.a_chirping_subscriber
    {
        ...
        It should_try_to_retrieve_an_existing_my_chirps = () => my_chirps_entity_context.Verify(ec => ec.GetById(has_chirped_before), Moq.Times.Once());
        It should_add_the_chirp_to_chirpers_chirps = () => my_chirps.Exists(chirp).ShouldBeTrue();
        It should_increment_the_total_count_for_the_chirpers_chirps = () => my_chirps.TotalNumberOfChirps.ShouldEqual(initial_chirp_count + 1);
        It should_update_my_chirps = () => my_chirps_entity_context.Verify(ec => ec.Update(my_chirps), Moq.Times.Once());
